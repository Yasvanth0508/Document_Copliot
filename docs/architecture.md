...
- Google Gemini (`google-genai` SDK) for generation and embeddings
...
- chunk text
- embedding vector (768 dimensions — `gemini-embedding-2` with `output_dimensionality=768`)
...
- `vector(768)` embedding columns if the SQLAlchemy type renderer is not sufficient (`gemini-embedding-2` with `output_dimensionality=768`)
...
- No direct Gemini / Google AI calls from the browser.
